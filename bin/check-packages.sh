...
nix_env=()
if [[ -n "${GENOC_NIXPKGS_PATH:-}" ]]; then
  nix_env=(env "NIX_PATH=nixpkgs=${GENOC_NIXPKGS_PATH}:${NIX_PATH:-}")
else
  root_channel="/nix/var/nix/profiles/per-user/root/channels/nixos"
  if [[ -e "$root_channel" ]]; then
    nix_env=(env "NIX_PATH=nixpkgs=${root_channel}:${NIX_PATH:-}")
  fi
fi
